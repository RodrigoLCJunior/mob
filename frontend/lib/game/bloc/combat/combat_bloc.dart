...
class CombatBloc extends Bloc<CombatEvent, CombatState> {
  ...
  Future<void> _onPlayCard(PlayCard event, Emitter<CombatState> emit) async {
    ...
    if (!state.isPlayerTurn || state.combat == null) {
      print(
        'CombatBloc - Cannot play card: isPlayerTurn=${state.isPlayerTurn}, '
    ...
    if (event.cardIndex < 0 ||
      ...
      print(
        'CombatBloc - Invalid card index or ID mismatch: '
    ...
    print(
      'CombatBloc - Card ${event.card.id} played, new maoAvatar size: ${updatedMaoAvatar.length}',
    );
